...
public class TruckEventProcessorKafkaTopologyPhase3 extends BaseTruckEventTopology {
	...
	public StormTopology buildTopology() {
		...
		/* Sliding window to caluclate speed */
	...
	private void configureSolrIndexingBolt(TopologyBuilder builder) {
		...
		if(isIndexingEnabled) {
			...
			builder.setBolt("solr_indexer_bolt", solrBolt, solrBoltCount).shuffleGrouping("Truck-Events-Stream");
		} else {
	...
	public void configureWebSocketBolt(TopologyBuilder builder) {
		...
		if(configureWebSocketBolt) {
			...
			builder.setBolt("Notification-Dashboard", webSocketBolt, 4).shuffleGrouping("Logistical-Regression-Score");
		}
	}

	public void configureHBaseBolt(TopologyBuilder builder) {
		TruckHBaseBolt hbaseBolt = new TruckHBaseBolt(topologyConfig);
		builder.setBolt("Logistical-Regression-Score", hbaseBolt, 2 ).shuffleGrouping("Join-Truck-Streams");
	}
	...
	public void configurePhoenixHBaseBolt(TopologyBuilder builder) {
		...
		builder.setBolt("Phoenix-Sink", hbaseBolt, 2 ).shuffleGrouping("Truck-Events-Stream");
	}	
	...
	public void configureSlidingWindowTruckAverageSpeed(TopologyBuilder builder) {
		int boltCount = Integer.valueOf(topologyConfig.getProperty("trucking.bolt.thread.count"));
		Duration windowLength = new Duration(5, TimeUnit.MINUTES);
	...
	public void configureJoinAverageSpeedAndInfractionCount(TopologyBuilder builder) {
		int boltCount = Integer.valueOf(topologyConfig.getProperty("trucking.bolt.thread.count"));
		Duration windowLength = new Duration(3, TimeUnit.MINUTES);
	...
	public void configureHDFSBolt(TopologyBuilder builder) {
		...
		builder.setBolt("hdfs_bolt", hdfsBolt, hdfsBoltCount).shuffleGrouping("Truck-Events-Stream");
	}
	...
	public int configureTruckSpeedEventsKafkaSpout(TopologyBuilder builder) {
		KafkaSpout kafkaSpout = constructTruckSpeedEventKafkaSpout();
	...
	private KafkaSpout constructTruckSpeedEventKafkaSpout() {
		KafkaSpout kafkaSpout = new KafkaSpout(constructTruckSpeedEventKafkaSpoutConf());
		return kafkaSpout;
	}	
	...
	private SpoutConfig constructTruckSpeedEventKafkaSpoutConf() {
		BrokerHosts hosts = new ZkHosts(topologyConfig.getProperty("kafka.zookeeper.host"));
